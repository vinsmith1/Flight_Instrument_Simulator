...
def ConvertTrackLog(input_csv_filename=None, framerate=30):
    ...
    Args:
        ...
        framerate (int, optional): _description_. Defaults to 30.
    """
    # Track time for benchmarking purpose
    benchmark_time_start = time.time()
    print(f'[ConvertTrackLog] {time.strftime("%Y-%m-%d %H:%M:%S")} Start processing {input_csv_filename}')

    # Validate input argument....just check that the file is a .csv
    if not str(input_csv_filename).endswith('.csv'):
        print('[convertTrackLog] Error: filename passed to this function must have .csv extension')
        return
    ...
    print(f'[ConvertTrackLog] {time.strftime("%Y-%m-%d %H:%M:%S")} Output file {output_csv_filename} contains {len(data["Timestamp"])} records') 
    print(f'[ConvertTrackLog] {time.strftime("%Y-%m-%d %H:%M:%S")} Finished in {time.time()-benchmark_time_start:#.1f} seconds')    

if __name__ == "__main__":
    ConvertTrackLog('test tracklog.csv')