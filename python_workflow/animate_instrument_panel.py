...
def animate_instrument_panel(tracklog_filename:str, frame_rate:int, output_image_folder:str, logging=False):
    ...
    if logging:
        write_dict_to_csv(data, output_image_folder + tracklog_filename.removesuffix('.csv') + '_output.csv')
    ...
    if logging:
        write_dict_to_csv(frames, output_image_folder + tracklog_filename.removesuffix('.csv') + '_frames.csv')
    ...
    tracklog_filename = 'tracklog.csv'
   
    # without profiling
    animate_instrument_panel(tracklog_filename, frame_rate, output_image_folder, logging=True)
    
    # with profiling
    # cProfile.run('animate_instrument_panel(tracklog_filename, frame_rate, output_image_folder)', 'profile_results')
